...
const schema = z.object({
    ...
    amount: z
        ...
        .min(0.01, {message: 'Item price should be at least 1 euro cent.'})
        .max(100_000, {message: 'Item price should not exceed 100 thousand.'}),
    category: z
...
const Form = ( {onSubmit}:Props ) => {
  ...
  return (
    ...
    <div className="mb-3">
        ...
        <input {...register('description')} id='description' type="string" className="form-control"></input>
        { errors.description && <p className='text-danger'>{ errors.description.message }</p> }
    </div>

    <div className="mb-3">
        <label htmlFor='amount' className="form-label">Item Price</label>
        <input {...register('amount', {valueAsNumber: true})} id = 'amount' type="number" className="form-control" min="0.00" step="0.01"></input>
